...
public class UserGetProcessor {
    ...
    public UserView process(UserFindRequest request){
        ...
        Optional<SessionResponseDTO> sessionResponseDTO = sessionService.findSession(request.getSessionId());
        AssertUtil.isTrue(sessionResponseDTO.isPresent(), "Session not found");

        UserResponseDTO userResponseDTO = userService.findUserById(sessionResponseDTO.get().getUserId())
